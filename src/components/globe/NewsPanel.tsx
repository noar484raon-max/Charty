...
const SENTIMENT_LABEL_KO: Record<string, string> = {
  positive: "긍정",
  negative: "부정",
  neutral: "중립",
};
...
function SentimentMeter({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-[14px] font-bold font-mono" style={{ color: getColor(clamped) }}>
        {clamped}
      </span>
    </div>
  );
...
function NewsArticleRow({ article }: { article: GlobalNewsItem }) {
  const dateStr = article.publishedAt
    ? new Date(article.publishedAt).toLocaleDateString("ko-KR", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })
    : "";
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1.5">
            ...
            <span className={`inline-flex items-center px-1.5 py-0 rounded text-[9px] font-semibold border ${SENTIMENT_BADGE[article.sentiment]}`}>
              {SENTIMENT_LABEL_KO[article.sentiment]}
            </span>
          </div>
          {article.description && (
  ...
  if (!countryCode) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center px-6 py-12">
        ...
        <div className="text-[15px] font-bold text-zinc-300 mb-1">
          국가를 선택하세요
        </div>
        <div className="text-[12px] text-zinc-500 max-w-[240px]">
          지구본에서 국가 마커를 클릭하면 해당 국가의 실시간 비즈니스 뉴스와 감성 분석을 볼 수 있습니다
        </div>
      </div>
    );
  }
      ...
      <div className="sticky top-0 bg-surface/95 backdrop-blur-sm border-b border-white/[0.06] px-4 py-3 z-10">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-[10px] text-zinc-500 mb-0.5">뉴스 감성</div>
            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-bold border ${
              data.overallSentiment >= 60 ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/30"
              : data.overallSentiment >= 45 ? "bg-blue-500/15 text-blue-400 border-blue-500/30"
        ...
        <div className="mt-2">
          <SentimentMeter score={data.overallSentiment} />
        </div>
      </div>
      ...
      <div className="px-4 py-2">
        {data.articles.length === 0 ? (
          ...
          data.articles.map((article, i) => (
            <NewsArticleRow key={i} article={article} />
          ))
        )}
      </div>
